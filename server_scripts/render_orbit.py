...
def main():
    ...
    trained_iter = 40000
    out_dir = "/workspace/gaussian_project/results/novel_orbit"
    ...
    centers = np.stack(
        [c.camera_center.detach().cpu().numpy() for c in train_cams], axis=0
    ).astype(np.float32)

    scene_center = centers.mean(axis=0)
    radius = float(np.linalg.norm(centers - scene_center[None, :], axis=1).mean() * 2.0)
        ...
        for i in tqdm(range(num_frames)):
            ...
            cam_pos = np.array(
                [
                    ...
                    scene_center[1] + height * np.sin(theta),
                    scene_center[2] + radius * np.sin(theta),
                ],
                dtype=np.float32,
            )
            ...
            cam.R = R_c2w
            cam.T = T
            cam.image_name = f"orbit_{i:04d}.png"
